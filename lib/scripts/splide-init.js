...
document.addEventListener('DOMContentLoaded', function () {
    console.log("网页加载完成 - DOMContentLoaded")
    splideLoadingHandler();
    initGlobalMediumZoom();
});

document.addEventListener('pjax:success', function () {
    console.log("网页加载完成 - pjax:success")
    splideLoadingHandler();
    initGlobalMediumZoom();
});
...
function initGlobalMediumZoom() {
    // 检查是否启用 medium-zoom
    if (!mediumZoomEnabled) {
        console.log('Medium Zoom 全局未启用，不执行初始化');
        return;
    }
    ...
    console.log('Medium Zoom 全局初始化完成');
}
...
function initCarouselMediumZoom() {
    ...
    console.log("Medium Zoom 轮播加载完成")
